...
class AllAuthPasswordResetForm(DefaultPasswordResetForm):
    def clean_email(self):
        """
        Invalid email shoudl not raise error, as this would leak users
    ...
    def save(self, request, **kwargs):
        ...
        for user in self.users:
            ...
            if getattr(settings, 'REST_AUTH_PW_RESET_USE_SITES_DOMAIN', False):
            ...
            context = {
                ...
                'user': user,
                'password_reset_uri': url,
                'request': request,
            }
            if app_settings.AUTHENTICATION_METHOD != app_settings.AuthenticationMethod.EMAIL:
                context['username'] = user_username(user)
            get_adapter(request).send_email(
